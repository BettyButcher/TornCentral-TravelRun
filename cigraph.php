...
$itemid = isset($_GET['it']) ? (1 * $_GET['it']) : 0;
...
if (!is_file($filename)) {
  ...
  #title
  $sql = "select country.countryname, country.countryid from country where country.letter = '$ccode'";
  $res = mysql_query($sql) or die(mysql_error());
  $countryname = mysql_result($res, 0, 0);
  $countryid = 1 * mysql_result($res, 0, 1);
  mysql_free_result($res);
  $sql = "select t.itemtypename, i.itemname from itemtype t, item i where t.itemtypeid = i.itemtype and i.itemid = $itemid";
  $res = mysql_query($sql) or die(mysql_error());
  ...
  $scaled = round(1000 / $scale, 0);
  imagestringup($im, 2, 1, 290, 'quantity * ' . $scaled, $black);
  ...
  #get data
  $sql = "select utctime, price, quantity from stock where country = $countryid and item = $itemid";
  $res = mysql_query($sql) or die(mysql_error());
  while ($row = mysql_fetch_row($res)) {
    ...
    $realy = (10000 - ($row[2] * $scale)) / 10000 * 420 + 40;
